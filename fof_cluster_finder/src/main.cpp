...
void Main::read_data () {
  ...
  if(opt.input_mode == "fits")
    fileio.read_fits(opt.input_file, opt.fof_mode, galaxies);
  else if(opt.input_mode == "ascii")
    fileio.read_ascii(opt.input_file, opt.fof_mode, galaxies);  
}
...
void Main::assign_linking_param () {
  ...
  double r_ref = pow(double(zbins[z_ref_index].count) / (opt.z_bin_size * zbins[z_ref_index].dvdz), 0.5) * opt.link_r;
  for(int i = 0; i < num_bins; i++)
...
void Main::find_friends () {
  ...
  for (int i = 0; i < nbins; i++) {
    FoF fof_bin(opt.link_r, opt.link_z, opt.fof_mode);
    fof_bin.friends_of_friends(tree, zbins[i], galaxies);
...
void Main::output_results () {
  ...
  fileio.output_file_names(opt.input_file, opt.fof_mode, opt.output_mode, opt.link_r, opt.link_z);
