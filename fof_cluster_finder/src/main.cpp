...
int main (int argc, char *argv[]) {
    ...
    std::cout << "tot galaxies " << run_code.galaxies.size() << std::endl;
    unsigned int min = -1;
    unsigned int max = 0;
    for(auto& gal : run_code.galaxies ){
        min = gal.in_cluster.size() < min? gal.in_cluster.size() : min;
        max = gal.in_cluster.size() > max? gal.in_cluster.size() : max;
    }

    std::cout << "in_cluster " << min << " " << max << std::endl;
