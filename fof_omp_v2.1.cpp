...
/** @file fof_omp_v2.1.cpp
 * @brief Friends-of-friends cluster detection algorithm. 
 * @author Samuel Farrens
 */

#include <omp.h>                  //*Include Open MP*//
#include "./include/include.hpp"  //*Include C++ packages and cosmology routines*//
#include "./include/fof.hpp"      //*Include FOF header file*//
...
//! Define integer vector for structures
typedef std::vector<int> intvec;           
//! Define double vector for structures 
typedef std::vector<double> doublevec;      
//! Define string vector for structures
typedef std::vector<std::string> stringvec; 

struct galaxy_clust{                //! Define Structue for Galaxies
...
struct protocluster{                //! Define Structue for Protoclusters
...
struct cluster{                     //! Define Structue for Clusters
  int id;                           /**< Cluster ID */
...
void cluster_finder::whileEOF(std::ifstream &inputfile,std::vector<std::string> &header,std::vector<std::string> &values,
			const std::string &comment_str){ //! Read each line in file
  std::string line;
...
void cluster_finder::open_log_file(){ //! Open log file
  struct tm *current;
  time_t now;
  time(&now);
...
void cluster_finder::read_data_file(const std::string &fname){ //! Read in data file and calculate R_friend(z)
  ...
  mean_particle_separation();  /**< Find mean particle separation */
  ...
  if(!strcmp(linking_method.c_str(),"dynamic")){
    ...
    r_temp=linking_length*pow((double)((R_fr.y(z_sliced[i_temp])/delta_z)*(1/dcmv[i_temp])),(double)0.5); /**< R Constant */
    for(int i=0;i<Nslices;i++){
    ...
    R_friend.push_back(linking_length);
...
void cluster_finder::mean_particle_separation(){ //! Calculate mean particle separation
  int count=0;
  double xxx;
  double sum_dist=0;
	...
	for(int j_node=0;j_node<(int)pow(2.0,(tree_cluster.MAX_DEPTH));j_node++){ /**< Loop over kd-tree nodes */
	  if(check_node(i,j_node)<frndrad){ /**< Check for nodes nearby current galaxy */
		...
		if(!strcmp(type.c_str(),"spec") && fabs(v1-v2)<=v_0[oint] || !strcmp(type.c_str(),"phot") 
		  ...
		  dist=proj_dist(gal[j].ra,gal[i].ra,gal[j].sindec,gal[i].sindec,
				 gal[j].cosdec,gal[i].cosdec); /**< Calculate distance between pair */
		  if(dist<frndrad){ /**< Check that distance is less than threshold */
		    ...
		    else if(gal[i].friends[z_oint]>-1){ /**< Add galaxy to existing proto-cluster */
		      gal[j].friends[z_oint]=k;
...
void cluster_finder::cluster_properties(){    //! Calculate basic physical properties of each cluster
  ...
  for(int mm=0;mm<clt.size();mm++){
    ...
    clt[mm].size=(double)(cltrad)*(180/M_PI)*3600;
